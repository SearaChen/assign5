...
void main()
{
    ...
    vec3 xPosition = texture(texturePosition,texCoords).xyz;
    vec3 xNormal = texture(textureNormal, texCoords).rgb;


    //2)Build the shading normal's frame (TBN).
    vec3 tangent = getTangent(xNormal);
    vec3 bitangent = cross(xNormal, tangent);
    mat3 TBN = mat3(tangent, bitangent,xNormal);

    float sum = 0;
    for (int i = 0 ; i < 32 ; i++)
    {
        // 1)Get a sample direction (view space)
        vec3 sample = normalize(squareToUniformHemisphere());
        ...
        sample = xPosition + TBN * sample * RADIUS;

        // 4) Get the depth value at the sample's position using the depth buffer.
        // project the sample to screen space (NDC)
        vec4 offset = vec4(sample, 1.0);
        offset = projection * offset ;
        offset. xyz /= offset.w;  // why is the offset 4 dimension
        offset.xyz = offset.xyz * 0.5 +0.5;    //Transform the sample in NDC coordinates [-1,1] to texture coordinates [0,1].

        float sampleDepth = texture(texturePosition , offset.xy).z;

        sum +=(sampleDepth <= sample.z ? 1.0 * cosTheta * INV_PI / squareToUniformHemispherePdf(): 0.0);
    }
    color = vec3(sum/ 32);
}
