...
void Renderer::render() {
    if (realTime) {
        ...
        for (int x = 0; x < imageWidth ; ++x)
        {
            ...
            {
                v3f pixelColorSum(0.0f, 0.0f, 0.0f);

                for (int i = 0; i < spp; ++i)
                {
                    float epsilonX =sampler.next();
                    float epsilonY =sampler.next();
                    ...
                    // render colour
                    pixelColorSum = integrator->render(ray , sampler) + pixelColorSum;
                }
                v3f pixelColor = pixelColorSum/((float)spp);
